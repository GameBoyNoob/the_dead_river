...
label mother_actions:
    menu:
        "Срочно отключить воду в районе (бесплатно)":
            $ rating += 1
            m "Спасибо! Хоть кто-то реагирует."
        "Организовать подвоз чистой воды (3000$)":
            if budget >= 3000:
                $ budget -= 3000
                $ rating += 2
                ...
                "У вас не хватает средств на подвоз чистой воды"
                "Оставшийся бюджет: [budget]"
                jump mother_actions
...
label case_factory_worker:
    ...
    "В кабинет заходит мужчина в спецовке, пахнет машинным маслом."
    "Рабочий" "Здравствуйте, я работаю на заводе у реки. Говорят, что мы виноваты в грязной воде, но у нас всё по правилам! Почему теперь нас хотят закрыть?"

label factory_worker_info
    menu:
        "Спросить, есть ли у завода очистные сооружения":
            "Рабочий" "Говорят, что есть, но я сам не видел. Иногда вода из трубы идёт мутная, но начальство говорит — всё нормально."
            jump factory_worker_info
        "Спросить, были ли аварии на производстве":
            "Рабочий" "Недавно был сбой — насос сломался, отходы пару часов шли прямо в реку. Но начальство велело молчать."
            jump factory_worker_info
        "Спросить, кто отвечает за экологию на заводе":
            "Рабочий" "У нас есть инженер по охране труда, но он только бумаги подписывает."
            jump factory_worker_info
        "Закончить расспросы":
            jump factory_worker_menu

label factory_worker_menu:
    menu:
        "Провести внеплановую проверку завода (1000$)":
            if budget >= 1000:
                $ budget -= 1000
                $ rating += 2
                "Рабочий" "Спасибо, что не закрываете сразу. Может, после проверки что-то изменится."
                "Оставшийся бюджет: [budget]"
            else:
                "Рабочий" "У вас не хватает средств на проверку."
                "Оставшийся бюджет: [budget]"
                jump factory_worker_menu
        "Потребовать отчёты и дать неделю на исправление":
            $ rating += 1
            "Рабочий" "Спасибо за шанс. Надеюсь, начальство прислушается."
        "Пригрозить штрафом и увольнением":
            $ rating -= 1
            "Рабочий" "Вот так всегда... Простых людей наказывают, а начальство ни при чём."
        "Пожать плечами и отпустить":
            $ rating -= 2
            "Рабочий" "Понятно... Никому мы не нужны."
...
label case_farmer_info:
    menu:
        ...
        "Спросить, кто поставщик химикатов":
            ...
            jump case_farmer_info
        "Закончить расспросы":
            jump case_farmer_menu

label case_farmer_menu:
    menu:
        "Обязать установить очистку (5000$ субсидия)":
            if budget >= 5000:
                $ budget -= 5000
                $ rating += 1
                f "Это дорого, но если надо — будем думать."
            else:
                "У вас не хватает средств на субсидию."
                "Оставшийся бюджет: [budget]"
                jump case_farmer_menu
        "Оштрафовать (штраф в бюджет +1000$)":
            $ budget += 1000
            $ rating -= 1
...
label check_reports_menu
    menu:
        "Вызвать инспекцию (500$)":
        if budget >= 500
            $ rating += 2
            $ budget -= 500
            a "Серьёзный шаг, но, возможно, правильный."
        else:
            "Вам не хватает бюджета на вызов инспекции"
            "Оставшийся бюджет: [budget]"
            jump check_reports_menu
