...
label day1_start:
    ...
    g "{cps=40}С самого детства я грезил футболом — не славой или деньгами, а самой игрой. Чистым азартом и командным духом.{/cps}"
    ...
    # Slower, more emotional line using text tag
    g "{cps=20}Но мое сердце принадлежало полю.{/cps}"
    ...
    # Passionate, slower speech for emotional content
    g "{cps=15}Футбол — это моя жизнь.{/cps} {i}(с тоской смотрит вдаль){/i}"
    ...
    # Bitter realization with varying speeds
    g "{cps=25}Но мои слова всегда понимали не так.{/cps}"
    ...
    # Slower for emphasis
    g "{cps=20}Мою страсть к дискуссиям на поле — как задатки юриста.{/cps}"

    show g sad

    # Using narrator_dramatic for slower, more dramatic narration
    ...
    g "{cps=10}Весь город постепенно превратился в декорации к фильму о киберпанке — трубы, смог, бетон.{/cps}"

    hide g
    with dissolve

    # Fast, announcer-style
    "Ведущий Новостей" "{cps=60}Будущее уже наступило!{/cps} {i}(громко, с фальшивым энтузиазмом){/i}"

    "Ведущий Новостей" "{cps=50}Наш город станет центром технологий будущего!{/cps}"

    "Ведущий Новостей" "Уже сейчас мы занимаем первое место по производству микрочипов!"

    "Ведущий Новостей" "{cps=45}Мы обещаем, что за следующие 8 лет качество вашей жизни вырастет на 40 процентов!{/cps} {i}(показывает диаграмму роста){/i}"

    show g talk
    with dissolve

    # Slower, thoughtful delivery
    g "{cps=20}А у меня душа болела за родной город. За реку, которая становилась всё темнее. За воздух, которым становилось всё труднее дышать.{/cps}"

    g "Я чувствовал, что эта гонка за прогрессом любой ценой до добра не доведёт."

    # Dramatic pause with slower text
    g "{cps=15}И, похоже, оказался прав.{/cps}"

    # Building tension with slower speed
    g "{cps=20}История повторяется, но теперь ставки куда выше — речь идёт не о технологической гонке, а о самом выживании города.{/cps}"

    hide g
    show g sad

    # Big reveal - slower typing
    g "{cps=15}Недавно выборы изменили всё: новым депутатом стал... я.{/cps}"
    
    g "И пусть я ещё не выучил наизусть все коридоры власти, одно знаю точно — настало время действовать."

    hide g

    # Scene transition with slow narrator
    narrator_dramatic "Первый рабочий день начинается."

    show a at left
    with dissolve

    # Assistant speaks at defined slower pace
    a "Георгий Николаевич, поздравляю с назначением! Я — Настя, ваш помощник. Мы оба здесь новенькие."
    
    # Dropping voice with slower typing
    a "{cps=15}Город действительно ждёт перемен...{/cps} {i}(понижает голос){/i} {cps=10}особенно с водой. Ситуация становится критической.{/cps}"
    ...
    p "{cps=45}Здравствуйте, Георгий Скороденок. Понимаю, вы только вступили в должность, но времени на раскачку нет — у нас чрезвычайная ситуация.{/cps}"
    ...
    # Serious news - slower for emphasis
    p "{cps=25}Вода по анализам признана непригодной не только для питья, но даже для бытового использования.{/cps}"

    p "{cps=20}Уже зарегистрированы случаи отравления — проблемы с кожей, желудком, есть подозрения на массовое отравление.{/cps}"
    ...
    # Urgent tone - faster
    p "{cps=40}Экологи уже выезжали на место. Люди в ярости, и я их понимаю.{/cps}"
    ...
    p "{cps=30}У вас три дня, Георгий. Ситуация накаляется с каждым часом. Население требует ответов.{/cps}"

    show g talk

    # Determined response
    g "{cps=35}Понял вас. Начинаем работать немедленно. До связи.{/cps}"
    ...
    # Question that leads to player choice
    a "Георгий, вчера мне писала Анна Петровна по поводу массовой гибели рыбы в реке. {i}(смотрит вопросительно){/i} {cps=15}С чего начнём?{/cps}"
